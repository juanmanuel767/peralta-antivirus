...
class Peralta:
    def __init__(self):
        ...
        self.estado_escaneo  = {"activo": False, "progreso": 0, "total": 0, "archivo": "", "amenazas": 0}
    ...
    def iniciar_daemon(self):
        ...
        Voz.hablar("Bienvenido. El motor de seguridad Peralta ha sido iniciado correctamente. El escudo neural está activo.", urgente=True)
    ...
    def escanear(self, ruta: str, profundo: bool = False):
        ...
        self.estado_escaneo = {"activo": True, "progreso": 0, "total": len(archivos), "archivo": "", "amenazas": 0}
        ...
        with Progress(SpinnerColumn(), TextColumn("{task.description}"),
            ...
            for archivo in archivos:
                ...
                if r["estado"] == "AMENAZA":
                    amenazas.append(r)
                    self.estado_escaneo["amenazas"] = len(amenazas)
                elif r["estado"] == "SOSPECHOSO":
                    sospechosos.append(r)
                self.logger.info(f"[{r['estado']}] {archivo}")
                self.estado_escaneo["progreso"] += 1
                self.estado_escaneo["archivo"] = str(Path(archivo).name)
                prog.advance(tarea)
                time.sleep(0.1)  # Simula el delaya de heurística para apreciación del UI
        ...
        if amenazas or sospechosos:
            ...
            Voz.hablar("Listo, escaneo completado.")
            
        self.estado_escaneo["progreso"] = self.estado_escaneo["total"]
        time.sleep(1.5)
        self.estado_escaneo["activo"] = False
...
"""
    ...
    def iniciar_servidor_web(self, puerto: int = 5005):
        ...
        try:
            ...
            def get_scan_status():
                return jsonify(self.estado_escaneo)
            ...
            def chat_ia():
                ...
                if any(x in mensaje for x in ["hola", "hello", "buenos", "buenas"]):
                    return jsonify({"respuesta": "¡Hola! Soy Peralta, tu núcleo de seguridad táctica. El motor neural profundo puede tardar en procesar lenguaje en este hardware. Te sugiero usar comandos directos instantáneos como: 'analiza', 'actualiza', o 'cuarentena'."})

                if any(x in mensaje for x in ["analiza", "escanea", "scan"]):
                    Voz.hablar("Se ha empezado a hacer un escaneo a profundidad")
                    # Ejecutar escaneo en segundo plano (en una carpeta con archivos para que el UI pueda mostrar progreso visual)
                    threading.Thread(target=self.escanear, args=("/home/user/Descargas/paguina web para antivirus/peralta-shield-main/src", False)).start()
